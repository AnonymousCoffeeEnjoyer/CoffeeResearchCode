...
def main() -> int:
    ...
    try:
        with open(output_path, "w", buffering=1, encoding="utf-8") as result_output:
            ...
            best_path = os.path.join(targetpath, "LSTM_model_best.pt")
            best_val = float("inf")
            ...
            for epoch in range(epochs):
                ...
                for pcs, deltas, delta_targets, pc_targets in train_iter:
                    ...
                    _, _, _, loss = model(pcs, deltas, delta_target=delta_t1, pc_target=pc_t1)
                    if loss is None:
                        continue
                ...
                v = val_losses.get("next1_total")
                if v is not None and float(v) < best_val:
                    best_val = float(v)
                    torch.save(model.state_dict(), best_path)
                    print(
                        f"Epoch {epoch + 1}: saved best checkpoint (next1_total={best_val:.6f}) to {best_path}"
                    )
