        ...
        with open(output_path, "w", buffering=1, encoding="utf-8") as result_output:
            ...
            for epoch in range(epochs):
                ...
                for pcs, deltas, delta_targets, _pc_targets in train_iter:
                    ...
                    _, loss = model(pcs, deltas, steps=horizon, targets=delta_targets)
                    if loss is None:
                        continue
                ...
                # Save best checkpoint by validation next-N CE (all horizon steps).
                current_val = float(val_losses.get("nextn", 0.0))
                if current_val < best_val_loss:
                    best_val_loss = current_val
                    torch.save(model.state_dict(), best_model_path)
                    print(f"Saved best model to {best_model_path} (val nextN={best_val_loss:.6f})")
